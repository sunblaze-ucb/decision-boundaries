...
# usage: python classify_test.py <modelname> <step> <adv_set> <classifier checkpoint>
num_classes = 10
import sys
modelname, step, adv_set, classifier_checkpoint = sys.argv[1:]
step = int(step)
...
def load_dist(setname):
    dist = load_seq('gxr3_%s/step%d_%s%%d_dist.npy' % (modelname, step, setname), 200)
...
def load_purity(setname):
    boundary = load_seq('gxr3_%s/step%d_%s%%d_boundary.npy' % (modelname, step, setname), 200)
