...
class CarliniL2Ensemble:
    def __init__(self, sess, model, batch_size=1, confidence = CONFIDENCE,
                 ...
                 initial_const = INITIAL_CONST, modifier_init = None, random_init = RANDOM_INIT,
                 noise_count = 20, noise_mag = 0.3):
        """
        ...
        # hack, only works when you do one batch, because modifier_init has to be batch_size long
        self.modifier_init = modifier_init
        ...
        if self.modifier_init is not None:
            pass
        ...
        # the resulting image, tanh'd to keep bounded from 0 to 1
        self.newimg = tf.tanh(modifier + self.timg) * 0.5 + 0.5
        ...
        self.l2dist = tf.reduce_sum(tf.square(self.newimg-(tf.tanh(self.timg) * 0.5 + 0.5)) / (255 * 255),[1,2,3])
    ...
    def attack_batch(self, imgs, labs):
        ...
        imgs = np.arctanh((imgs - 0.5) / 0.50001)
