...
checkpoint_dirs = {
    ...
    'madry_nat': 'models/natural/checkpoint-%d'
}
...
dist = np.zeros(num_dirs * 2, dtype=np.float32)
boundary = np.zeros(num_dirs * 2, dtype=np.uint8)
...
for step in steps:
    ...
    for j in tqdm.trange(num_images, leave=False):
        ...
        running = np.arange(num_dirs * 2, dtype=np.int32)
